    ...
    <div className='rounded-lg border bg-card shadow-sm'>
      ...
      <div className='flex flex-wrap items-start justify-between gap-3 border-b px-5 py-4'>
        <div className='flex items-start gap-3'>
          ...
          <div>
            <h3 className='text-base font-semibold leading-tight'>{classroom.name}</h3>
        ...
        <div className='flex items-center gap-2'>
          {classroom.active ? (
            <Badge className='gap-1 bg-green-500/15 text-green-600 hover:bg-green-500/15 border-green-500/30'>
          ...
          {classroom.dormitory && (
            <Badge variant='outline' className='gap-1'>
              <Building2 size={11} />
              {classroom.dormitory.name ?? `${classroom.dormitory.gender} Lv.${classroom.dormitory.level}`}
            </Badge>
          )}
        </div>
      </div>
...
function OverviewTab({ classroom }: { classroom: Classroom }) {
  const rows: { label: string; value: React.ReactNode; icon: React.ReactNode }[] = [
    ...
    },
    {
      ...
      value: classroom.teacher || <span className='italic text-muted-foreground'>—</span>,
    },
    {
  ...
  return (
    <div>
      <p className='mb-3 text-xs text-muted-foreground'>
        Showing <span className='font-semibold text-foreground'>{total}</span> active student{total !== 1 ? 's' : ''}
      </p>
          ...
          <tbody>
            {students.map((sh, idx) => (
              ...
              >
                ...
                <td className='px-3 py-2 font-mono text-xs'>{sh.student?.nis ?? '—'}</td>
                <td className='px-3 py-2 font-medium'>{sh.student?.name ?? '—'}</td>
                ...
                <td className='px-3 py-2 text-xs text-muted-foreground'>
                  {sh.startDate ? new Date(sh.startDate).toLocaleDateString('id-ID') : '—'}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  if (schedules.length === 0) {
    return (
      <div className='flex flex-col items-center gap-2 py-12 text-center'>
        ...
        <p className='text-sm text-muted-foreground'>No active schedules for this class.</p>
      </div>
    )
  }
  ...
  return (
    <div className='flex items-center gap-3 rounded-md border px-3 py-2'>
      ...
      <div className='flex w-20 shrink-0 flex-col items-center justify-center rounded bg-muted/60 px-2 py-1 text-center'>
        {slot ? (
          <>
            <span className='text-[10px] font-semibold text-muted-foreground'>Slot {slot.slot}</span>
